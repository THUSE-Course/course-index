...
你需要在 SECoder GitLab 上新建名为 `2026-Django-HW` 的项目并按要求将密钥在 CI/CD 变量中提供。
...
- 工作目录为 `/app`；
- 将源代码复制到工作目录；
- 安装 `requirements.txt` 声明的依赖并通过换源到 `https://pypi-cache-sepi.app.spring26a.secoder.net/simple` 加速下载；
- 对外暴露 80 端口；
- 容器运行时执行 `start.sh` 脚本。
...
!!! tip "Secoder Pypi 镜像"
    ...
    你可以通过 `https://pypi-cache-sepi.app.spring26a.secoder.net/simple` 镜像下载 Python 包。这个镜像是对 TUNA 镜像的本地缓存，这样做可以显著减轻 Secoder 对 TUNA 镜像的访问压力，同时能够进一步加速下载（常常在 3s 内可以完成依赖下载的步骤）。
...
!!! warning "请务必完成本任务"
    ...
    尽管此任务不直接影响提交流程，但由于无法将后端小作业部署到 SECoder 平台上，如没有在本地进行测试，后续排查问题会比较困难。
...
!!! info "在 SECoder 上测试"
    ...
    请在作业截止前检查你的容器是否运行着前端小作业，否则你将无法确认前端小作业运行情况是否正常。
