...
游戏记录指的是**某一时刻棋盘上所有细胞的状态记录**。另外，每一条游戏记录应当同时记录下述辅助信息：
...
自动播放状态下，除停止自动播放按钮与跳转按钮外，包括棋盘在内所有按钮、文本框等可交互组件均应**不**可用。交互行为描述如下：
...
自动播放状态下，除停止自动播放按钮与两个跳转按钮外，包括棋盘在内所有按钮、文本框等可交互组件均应**不**可用。交互行为与 Free Mode 完全一致。
...
- 页面顶部显示该用户的用户名
- 页面顶部增加一个回到整体记录列表页面的按钮、一个删除该用户所有游戏记录的按钮
