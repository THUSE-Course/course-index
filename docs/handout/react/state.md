...
建议在截止时间前最后一次部署后，在游戏主界面进行如下检查：

- 在初始条件下，任意点击若干个不同的细胞，这些细胞均应该变为存活状态，颜色也应该相应改变
- 连续点击某一个细胞，该细胞应该不断在存活与死亡状态之间切换，颜色也应该相应改变
- 在自由状态通过点击棋盘设置好某一起始状态后点击 Start auto play 按钮能够进入自动播放状态，棋盘每次自动演变的间隔需要在 1 秒内（需求文档要求为 300 ms，检查时因无法精确计时故只要约在 1 秒内即可）
- 在自动播放状态下，点击 Stop auto play 按钮能够退回自由状态，自动演变停止，点击棋盘，其响应点击事件

Step 3 的一部分与 Step 1 可以通过自动化测试进行检查。在正确完成 CI/CD 小作业的基础上，你应当能在 SECoder Gitlab CI 上观察到单元测试所输出的信息。如果完全通过，显示应当类似于：
...
只要上述信息中显示 Step 3 测试通过，即表示本 Step 的自动化检查已通过。

若你未正确完成 CI/CD 小作业导致无法通过上述方式检查，可以克隆仓库并本地运行单元测试进行自测。
