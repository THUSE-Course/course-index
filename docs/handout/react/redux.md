...
建议在截止时间前最后一次部署后检查游戏主界面是否满足：

- 未登录时，在自由状态下任意创建一个游戏记录，输入游戏记录名后点击保存按钮，提示需要登录并自动跳转至登录页面
- 输入新用户名和密码，点击注册/登录，提示成功并自动切换回到主界面
- 先前创建的游戏记录保持在棋盘上，并且最下方的跳转至登录页面按钮转变为已登录提示文字和注销按钮
- 再次输入游戏记录名点击保存，此时显示保存成功

由于本 Step 涉及到网络请求，如果出现网络错误提示弹窗或者 5 秒内页面没有反应，建议先重试一次并检查后端服务与网络环境。
