...
建议在截止时间前最后一次部署后检查是否满足：

- 最初进入游戏记录列表页面时，首先显示加载中提示文字，之后显示所有游戏记录
- 点击退回到主界面按钮后，随机存储若干个游戏记录，再进入游戏记录列表页面后，能够显示出这些记录
- 点击某一条记录的 Play it 按钮能够正确跳转并回放该记录
- 点击某一条记录的 View this user 按钮能够正确跳转并显示该用户的所有记录
- 记录列表中仅有自己创建的游戏记录显示有 Delete it 按钮

建议按照顺序检查上述五项，五项检查相互独立，靠前的检查项不影响靠后的检查项，除非其导致检查无法继续进行（如不存在相应的按钮等）。
