...
如果你在编写游戏的核心逻辑上遇到了困难，可以打开 `src/tests/step.test.ts` 文件，其中的 `stepBoardGT` 函数是我们提供的样例实现和对拍基准实现，可供本 Step 的参考。
...
Step 1 与 Step 3 的一部分可以通过自动化测试进行检查。在正确完成 CI/CD 小作业的基础上，你应当能在 SECoder Gitlab CI 上观察到单元测试所输出的信息。如果完全通过，显示应当类似于：
...
只要上述信息中显示 Step 1 测试通过，即表示本 Step 的核心逻辑已经通过自动化检查。

若你未正确完成 CI/CD 小作业导致无法通过上述方式检查，可以克隆仓库并在本地运行单元测试进行自测。
